...
cartController.getAllCarts = async (req, res) => {
    try {
        ...
        const carts = await cartModel.find().populate("customerId","name email")
...
cartController.getCartById = async (req, res) => {
    try {
        const cart = cartModel.findById(req.params.id).populate("customerId","name email")
        .populate("products.productId", "name price")
...
cartController.insertCart = async (req, res) => {
    try {
        ...
        // 2. Calcular el subtotal y el total
        // Variable para guardar el total a pagar
        let total = 0
        // Arreglo de productos
        let newProducts = [];
 
        // De todos los productos, se va a recorrer uno por uno
        // calculando el subtotal y el total a pagar
        for (let i = 0; i < products.length; i++){
            // Buscamos el producto en la base de daos
